...
export default {
  ...
  computed: {
    ...mapState({
      getinfoListEc: state => state.getInfo.getinfoListEc,
      getinfoDetailEc: state => state.getInfo.getinfoDetailEc
    })
  },
  methods: {
    onSubmit(formName) {
    ...
    GetList() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          // 查询
          this.listdata = []
          this.$store.dispatch('getInfo/getinfoListEc', payload).then(res => {
            this.loading = false
            if (res.success) {
              if (res.data) {
    ...
    getDetail(item) {
      ...
      this.$store.dispatch('getInfo/getinfoDetailEc', payload).then(res => {
